...
readonly class PasswordAttributes implements ArgumentInterface
{
    ...
    private const ONLY_DIGITS_PATTERN = '^(?=.*\d)[\d]{%s,}$';
    private const UPPERCASE_DIGITS_PATTERN = '^(?=.*[A-Z])(?=.*\d)[A-Z\d]{%s,}$';
    private const UPPERCASE_LOWERCASE_DIGITS_PATTERN = '^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{%s,}$';
    private const UPPERCASE_LOWERCASE_DIGITS_SPECIAL_CHARS_PATTERN = '^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_])[A-Za-z\d\W_]{%s,}$';
